...
(function(){
  ...
  function render(){
    ...
    body.innerHTML=records.filter(function(r){return Number(r.quantCompra||0)>0}).map(function(r){
      var nums=Array.isArray(a[r.id])?a[r.id]:[], available=nums.filter(function(n){return !sold[String(n)]}), c=typeof calc==='function'?calc(r):{saldo:r.quantCompra||0};
      return '<tr><td>'+esc(r.data)+'</td><td>'+esc(r.vendedor)+'</td><td>'+esc(r.era)+'</td><td>'+available.length+'</td><td>'+esc(available.join(', ')||'Nenhum disponível')+'</td><td><button class="mini" data-num-lote="'+esc(r.id)+'">Numerar</button></td></tr>';
  ...
  function init(){
    ...
    var sel=document.getElementById('animalLotV121');if(sel&&typeof records!=='undefined'){sel.innerHTML='<option value="">Selecionar lote (opcional)</option>'+records.filter(function(r){return Number(r.quantCompra||0)>0}).map(function(r){return '<option value="'+esc(r.id)+'">'+esc(r.data)+' — '+esc(r.vendedor)+' — '+esc(r.era)+'</option>'}).join('')}
