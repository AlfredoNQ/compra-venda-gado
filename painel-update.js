...
(function () {
  ...
  function ensureQuotePanel() {
    ...
    const grid = painel.querySelector('.grid2');
    ...
    const btn =
      document.getElementById('refreshCotacoes');

    if (btn) {
      btn.onclick = loadCotacoes;
    }
  }


  /* =========================
     BLOCO DE CADA ESTADO
  ========================= */
  ...
  async function loadCotacoes() {
    ...
    const el =
      document.getElementById('cotacoesConteudo');
    ...
    try {
      ...
      const pa = data.cotacoes.filter(function(x) {
        return x.uf === 'PA';
      });

      const to = data.cotacoes.filter(function(x) {
        return x.uf === 'TO';
      });

      const ma = data.cotacoes.filter(function(x) {
        return x.uf === 'MA';
      });
      ...
      el.innerHTML = `
        ...
        <div class="quote-meta quote-footer">
      ...
      el.innerHTML = `
        ...
        <div class="quote-error">
          ...
          Não foi possível carregar a cotação agora.
          Tente novamente pelo botão Atualizar.
  ...
  function initPainelUpdate() {
    ...
    const py =
      document.getElementById('panelYear');

    if (py) {
      py.onchange = renderKpis;
    }
