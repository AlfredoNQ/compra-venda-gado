...
(function(){
 ...
 function normalizeEditModal(){
  ...
  var list=(typeof records!=='undefined'&&Array.isArray(records)?records:(window.records||[]));var r=list.find(function(x){return x.id===(rid&&rid.value)}),isVenda=!!(r&&(window.__negListView==='buyer'||(Number(r.quantVenda||0)>0&&!Number(r.quantCompra||0))));
 ...
 function addSalesAt(){
  if((window.__negListView||'seller')!=='buyer')return;var h=document.querySelector('#negTableHead tr');if(!h||h.querySelector('[data-sales-at]'))return;var th=document.createElement('th');th.textContent='@';th.dataset.salesAt='1';h.insertBefore(th,h.children[6]);var wh=document.createElement('th');wh.textContent='Peso kg';wh.dataset.salesWeight='1';h.insertBefore(wh,th);var list=(typeof records!=='undefined'&&Array.isArray(records)?records:(window.records||[]));document.querySelectorAll('#tbody tr').forEach(function(tr){var edit=tr.querySelector('button[onclick*="editRecord"]'),id=edit&&((edit.getAttribute('onclick')||'').match(/editRecord\('([^']+)/)||[])[1],r=list.find(function(x){return x.id===id})||{},kg=Number(r.pesoVendaKg||r.pesoKg||r.peso||0),td=document.createElement('td'),at=document.createElement('td');td.className='num';td.textContent=num.format(kg);at.className='num';at.textContent=num.format(kg/30);tr.insertBefore(td,tr.children[6]);tr.insertBefore(at,tr.children[7]);});
 }
 function allocatedFromLot(id){return (window.records||[]).reduce(function(total,sale){var lots=sale.animalLotsSold||sale.sourceLots||[];return total+(Array.isArray(lots)?lots.reduce(function(a,x){return a+((x.lotId||x.id)===id?Number(x.quantity||x.quantidade||0):0)},0):0)},0)}
 function refreshAllocatedStock(){var body=document.getElementById('stockBody');if(!body)return;var list=(window.records||[]).filter(function(r){var saldo=Number(r.quantCompra||0)-Number(r.quantVenda||0)-allocatedFromLot(r.id);return saldo>0&&Number(r.quantCompra||0)>0}).sort(function(a,b){return (b.data||'').localeCompare(a.data||'')});body.innerHTML=list.map(function(r){var qc=Number(r.quantCompra||0),qv=Number(r.quantVenda||0),allocated=allocatedFromLot(r.id),saldo=Math.max(0,qc-qv-allocated),kg=Number(r.pesoKg||r.peso||0),pc=Number(r.precoCompra||0);return '<tr><td>'+fmtDate(r.data)+'</td><td>'+esc(r.vendedor||'')+'</td><td>'+esc(r.era||'—')+'</td><td class="num">'+num.format(qc)+'</td><td class="num">'+num.format(qv+allocated)+'</td><td class="num"><b>'+num.format(saldo)+'</b></td><td class="num">'+num.format(daysInStock(r))+'</td><td class="num">'+num.format(kg)+'</td><td class="num">'+money.format(pc)+'</td><td class="num">'+money.format(kg?pc/kg:0)+'</td><td class="num">'+money.format(saldo*pc)+'</td><td><span class="badge warn">'+(saldo===0?'Vendido':'Em estoque')+'</span></td><td>'+esc(r.parceiro||'—')+'</td></tr>';}).join('');}
 function run(){
 ...
 document.addEventListener('submit',function(ev){var f=ev.target;if(!f||f.id!=='recordForm'||window.__modeV127!=='venda')return;var total=Number(document.getElementById('rqv')?.value||0),sum=0;document.querySelectorAll('#loteRows130 .field').forEach(function(row){var l=row.querySelector('.lote130'),q=row.querySelector('.qtdLote130');if(l&&l.value)sum+=Number(q&&q.value||0)});if(!sum&&window.__v131PendingLots)sum=window.__v131PendingLots.reduce(function(a,x){return a+Number(x.quantidade||x.quantity||0)},0);if(total!==sum){ev.preventDefault();ev.stopImmediatePropagation();alert('A quantidade da venda deve ser igual à soma dos animais dos lotes. Venda: '+total+' • Lotes: '+sum);}},true);
 document.addEventListener('submit',function(ev){if(ev.target&&ev.target.id==='recordForm'&&window.__modeV127==='venda'){setTimeout(function(){var chosen=[];document.querySelectorAll('#loteRows130 .field').forEach(function(row){var l=row.querySelector('.lote130'),q=row.querySelector('.qtdLote130');if(l&&l.value&&Number(q&&q.value)>0)chosen.push({lotId:l.value,quantity:Number(q.value)});});if(!chosen.length&&window.__v131PendingLots)chosen=window.__v131PendingLots;var id=document.getElementById('rid')?.value,r=(window.records||[]).find(function(x){return x.id===id})||((window.records||[]).slice(-1)[0]);if(r&&chosen.length){r.animalLotsSold=chosen;r.sourceLots=chosen;try{persist()}catch(e){}}},800)}},false);
 ...
 setInterval(function(){var m=document.getElementById('modalTitle');if(m&&((m.textContent||'').indexOf('Editar')>=0||(m.textContent||'').indexOf('Nova')>=0)){normalizeEditModal();normalizePaymentCards();}},300);
