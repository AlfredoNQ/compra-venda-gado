...
(function(){
 ...
 function normalizePaymentCards(){var totals=document.getElementById('installmentTotals');if(!totals)return;var sale=window.__modeV127==='venda',cards=Array.from(totals.querySelectorAll('div')).filter(function(e){var t=(e.textContent||'').trim().toLowerCase();return (t.indexOf('a pagar ao vendedor')===0||t.indexOf('a receber do comprador')===0)&&e.children.length>0;});cards.forEach(function(card){var t=(card.textContent||'').trim().toLowerCase();card.style.display=(sale?t.indexOf('a receber do comprador')===0:t.indexOf('a pagar ao vendedor')===0)?'':'none';});}
