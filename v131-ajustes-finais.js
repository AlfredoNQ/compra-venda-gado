...
(function(){
 ...
 function normalizeEditModal(){
  ...
  var gc=document.getElementById('rgtaCompra'),gn=document.getElementById('rnotaCompra');if(gc)gc.value=(document.getElementById('rgta')||{}).value||'';if(gn)gn.value=(document.getElementById('rnota')||{}).value||'';var cv=document.getElementById('rcategoriaVenda'),cc=document.getElementById('rcategoria');if(cv&&cc)cv.value=cc.value||'';
 ...
 function addSalesAt(){
  if((window.__negListView||'seller')!=='buyer')return;var h=document.querySelector('#negTableHead tr');if(!h||h.querySelector('[data-sales-at]'))return;var th=document.createElement('th');th.textContent='@';th.dataset.salesAt='1';h.insertBefore(th,h.children[6]);var list=(typeof records!=='undefined'&&Array.isArray(records)?records:(window.records||[]));document.querySelectorAll('#tbody tr').forEach(function(tr){var edit=tr.querySelector('button[onclick*="editRecord"]'),id=edit&&((edit.getAttribute('onclick')||'').match(/editRecord\('([^']+)/)||[])[1],r=list.find(function(x){return x.id===id})||{},td=document.createElement('td'),kg=Number(r.pesoKg||r.peso||0);td.className='num';td.textContent=num.format(kg/30);tr.insertBefore(td,tr.children[6]);});
 }
 function run(){
