/* Compra e Venda de Gado — v94 fixes */
(function(){
  window.APP_WEB_VERSION='94';
  ...
  window.fileLink=function(doc,label){if(!doc||!doc.data)return '—';var id='pdf94_'+Math.random().toString(36).slice(2)+Date.now().toString(36);window.__pdfDocsV85[id]=doc;return '<button type="button" class="mini" onclick="openStoredPdfV85(\''+id+'\')">Abrir PDF</button>';};
  ...
  var TOMBSTONE_KEY='gado_deleted_records_permanent_v94';
  ...
  function forceVersion(){try{document.body.setAttribute('data-app-version','94');document.title='Compra e Venda de Gado — v94';var h=document.querySelector('header h1');if(h){var spans=h.querySelectorAll('span');var found=false;spans.forEach(function(s){if(/^v\d+$/i.test((s.textContent||'').trim())){s.textContent='v94';found=true;}});if(!found){var b=document.createElement('span');b.textContent='v94';b.style.cssText='font-size:12px;font-weight:800;padding:3px 7px;border-radius:999px;background:rgba(255,255,255,.16);vertical-align:middle;white-space:nowrap;margin-left:6px';h.appendChild(b);}}}catch(e){}}
