/* Compra e Venda de Gado — v93 fixes */
(function(){
  window.APP_WEB_VERSION='93';
  ...
  window.fileLink=function(doc,label){if(!doc||!doc.data)return '—';var id='pdf93_'+Math.random().toString(36).slice(2)+Date.now().toString(36);window.__pdfDocsV85[id]=doc;return '<button type="button" class="mini" onclick="openStoredPdfV85(\''+id+'\')">Abrir PDF</button>';};
  ...
  /* v93 — tombstones sincronizados pela própria coluna records. */
  var TOMBSTONE_KEY='gado_deleted_records_permanent_v92';
  var MARK='__deleted_record__:';
  ...
  function marker(id){return {id:MARK+id,__deletedRecordId:id,__deleted:true,updatedAt:new Date().toISOString()};}
  function markerId(r){return r&&r.__deleted===true&&r.__deletedRecordId?r.__deletedRecordId:(r&&typeof r.id==='string'&&r.id.indexOf(MARK)===0?r.id.slice(MARK.length):'');}
  function applyDeleted(extra){var all=tombstones().concat(extra||[]);saveTombstones(all);var d=new Set(all);records=records.filter(function(r){return !d.has(r&&r.id)&&!markerId(r);});try{userSet(KEY,JSON.stringify(records));}catch(e){}return all;}

  window.delRecord=function(id){if(!confirm('Excluir esta negociação?'))return;addTombstone(id);try{addDeletedId(DELETED_RECORDS_KEY,id);}catch(e){}try{costs.filter(function(c){return c.recordId===id;}).forEach(function(c){addDeletedId(DELETED_COSTS_KEY,c.id);});}catch(e){}records=records.filter(function(x){return x.id!==id;});costs=costs.filter(function(c){return c.recordId!==id;});try{markOfflineDirty();}catch(e){}persist();renderAll();try{autoExcelBackup();}catch(e){}if(navigator.onLine)setTimeout(function(){cloudSaveNow();},100);};
  ...
  cloudSaveNow=async function(){
    ...
    try{
      ...
      var cloudRecords=records.slice().concat(dels.map(marker));
      var payload={user_id:cloudUser.id,records:cloudRecords,costs:costs,updated_at:new Date().toISOString()};
  ...
  cloudLoad=async function(){
    ...
    cloudBusy=true;
    try{
      var res=await sb.from(CLOUD_TABLE).select('records,costs,updated_at').eq('user_id',cloudUser.id).maybeSingle();
      if(res.error)throw res.error;
      if(!res.data){cloudBusy=false;await cloudSaveNow();return;}
      var raw=Array.isArray(res.data.records)?res.data.records:[];
      var remoteDeleted=raw.map(markerId).filter(Boolean);
      var deleted=applyDeleted(remoteDeleted);
      var cloudRecords=raw.filter(function(r){return !markerId(r);});
  ...
  function forceVersion(){try{document.body.setAttribute('data-app-version','93');document.title='Compra e Venda de Gado — v93';var h=document.querySelector('header h1');if(h){var spans=h.querySelectorAll('span');var found=false;spans.forEach(function(s){if(/^v\d+$/i.test((s.textContent||'').trim())){s.textContent='v93';found=true;}});if(!found){var b=document.createElement('span');b.textContent='v93';b.style.cssText='font-size:12px;font-weight:800;padding:3px 7px;border-radius:999px;background:rgba(255,255,255,.16);vertical-align:middle;white-space:nowrap;margin-left:6px';h.appendChild(b);}}}catch(e){}}
