/* Compra e Venda de Gado — v90 fixes */
(function(){
  window.APP_WEB_VERSION='90';
  ...
  window.fileLink=function(doc,label){if(!doc||!doc.data)return '—';var id='pdf90_'+Math.random().toString(36).slice(2)+Date.now().toString(36);window.__pdfDocsV85[id]=doc;return '<button type="button" class="mini" onclick="openStoredPdfV85(\''+id+'\')">Abrir PDF</button>';};
  ...
  async function recoverSession(tryNo){
    ...
    try{
      if(typeof sb!=='undefined'&&sb&&navigator.onLine){
        ...
        if(s&&s.user){
          try{cloudUser=s.user;}catch(e){}
          try{bootLoginApproved=true;}catch(e){}
          try{if(typeof rememberAuthenticatedUser==='function')rememberAuthenticatedUser(s.user);}catch(e){}
          keepUi();
          try{if(typeof setCloudStatus==='function')setCloudStatus('Reconectado • sincronizando…','warn');}catch(e){}
    ...
    if(Date.now()<reconnectUntil&&(tryNo||0)<20){setTimeout(function(){recoverSession((tryNo||0)+1);},500);}
    return false;
  }

  window.initCloud=async function(){
    if(Date.now()<reconnectUntil){try{bootLoginApproved=true;}catch(e){}keepUi();recoverSession(0);return;}
    if(typeof originalInitCloud==='function')return originalInitCloud();
  };

  window.addEventListener('online',function(ev){
    reconnectUntil=Date.now()+20000;
    try{bootLoginApproved=true;}catch(e){}
    try{ev.stopImmediatePropagation();}catch(e){}
    keepUi();
    setTimeout(function(){recoverSession(0);},100);
  },true);

  window.addEventListener('pageshow',function(){
    try{if(typeof cloudUser!=='undefined'&&cloudUser){bootLoginApproved=true;keepUi();}}catch(e){}
  },true);

  function forceVersion(){try{document.body.setAttribute('data-app-version','90');document.title='Compra e Venda de Gado — v90';var h=document.querySelector('header h1');if(h){var spans=h.querySelectorAll('span');var found=false;spans.forEach(function(s){if(/^v\d+$/i.test((s.textContent||'').trim())){s.textContent='v90';found=true;}});if(!found){var b=document.createElement('span');b.textContent='v90';b.style.cssText='font-size:12px;font-weight:800;padding:3px 7px;border-radius:999px;background:rgba(255,255,255,.16);vertical-align:middle;white-space:nowrap;margin-left:6px';h.appendChild(b);}}}catch(e){}}
