...
public class MainActivity extends Activity {
        ...
        web.setWebViewClient(new WebViewClient() {
            ...
            @Override public WebResourceResponse shouldInterceptRequest(
                ...
                return isOnline()
                    ...
                    : offlineResponse(request.getUrl());
            }
    ...
    private void openOnlineWithFreshLogin() {
        String bootstrap =
            ...
            "<style>body{margin:0;background:#eef3ef;font-family:Arial,sans-serif}" +
            ".box{display:flex;min-height:100vh;align-items:center;justify-content:center;" +
            "color:#17633f;font-weight:700}</style></head><body>" +
            "<div class='box'>Protegendo acesso...</div>" +
            "<script>(function(){" +
            "try{" +
            "for(var i=localStorage.length-1;i>=0;i--){" +
            "var k=localStorage.key(i)||'';" +
            "if(/^sb-.*-auth-token$/i.test(k)||k.indexOf('supabase.auth.token')>=0){" +
