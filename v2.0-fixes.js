...
(function(){
  ...
  window.deleteNegotiationPdfV91=function(kind){try{var id=(document.getElementById('rid')||{}).value||'';if(!id){alert('Salve a negociação antes de excluir um PDF.');return;}var map={gta:{field:'gtaPdf',status:'rgtaFileStatus',input:'rgtaFile',label:'GTA'},nota:{field:'notaPdf',status:'rnotaFileStatus',input:'rnotaFile',label:'Nota'},pagamento:{field:'paymentPdf',status:'rpayFileStatus',input:'rpayFile',label:'Comprovante'}};var cfg=map[kind];if(!cfg)return;var r=records.find(function(x){return x.id===id;});if(!r)return;if(!r[cfg.field]){var st=document.getElementById(cfg.status);if(st)st.innerHTML='';return;}if(!confirm('Excluir somente o PDF de '+cfg.label+'?'))return;r[cfg.field]=null;r.updatedAt=new Date().toISOString();var inp=document.getElementById(cfg.input);if(inp)inp.value='';try{var deleted=JSON.parse((window.userGet?userGet('gado_pdf_deletions_v20'):localStorage.getItem('gado_pdf_deletions_v20'))||'{}');deleted[id+'|'+(kind==='pagamento'?'payment':kind)]=true;if(window.userSet)userSet('gado_pdf_deletions_v20',JSON.stringify(deleted));else localStorage.setItem('gado_pdf_deletions_v20',JSON.stringify(deleted));}catch(e){}var st=document.getElementById(cfg.status);if(st)st.innerHTML='<span style="color:#6d786f">PDF excluído</span>';persist();renderAll();if(typeof window.syncPendingNow==='function')setTimeout(function(){window.syncPendingNow(false);},100);}catch(e){alert('Não foi possível excluir o PDF: '+(e.message||e));}};
  ...
  async function loadSeparatedPdfs(list){
    ...
    if(res.error)throw res.error;
    var deleted={};try{deleted=JSON.parse(userGet(PDF_DELETE_KEY)||'{}')||{};}catch(e){}
    var map={};(res.data||[]).forEach(function(x){if(!deleted[String(x.record_id)+'|'+x.kind])map[String(x.record_id)+'|'+x.kind]=x.document;});
    return (Array.isArray(list)?list:[]).map(function(r){
      if(!r||!r.id)return r;
  ...
  async function save96(){
    ...
    try{
      ...
      // Mantém exclusões marcadas no formulário antes de reidratar PDFs antigos.
      var pdfDeleted={};try{pdfDeleted=JSON.parse(userGet(PDF_DELETE_KEY)||'{}')||{};}catch(e){pdfDeleted={};}
      records.forEach(function(r){if(!r||!r.id)return;[['gta','gtaPdf'],['nota','notaPdf'],['payment','paymentPdf']].forEach(function(p){if(pdfDeleted[String(r.id)+'|'+p[0]])r[p[1]]=null;});});
...
(function(){
 ...
 function allocatedFromLot(id,excludeId){return sourceRecords().reduce(function(total,sale){if(excludeId&&sale.id===excludeId)return total;var lots=sale.animalLotsSold||sale.sourceLots||[],used=Array.isArray(lots)?lots.reduce(function(a,x){return a+((x.lotId||x.id)===id?Number(x.quantity||x.quantidade||0):0)},0):0,saleTotal=Number(sale.quantVenda||0);return total+(saleTotal>0?Math.min(used,saleTotal):used)},0)}
 function refreshAllocatedStock(){var body=document.getElementById('stockBody');if(!body)return;var list=sourceRecords().filter(function(r){var saldo=Number(r.quantCompra||0)-allocatedFromLot(r.id);return saldo>0&&Number(r.quantCompra||0)>0}).sort(function(a,b){return (b.data||'').localeCompare(a.data||'')});body.innerHTML=list.map(function(r){var qc=Number(r.quantCompra||0),allocated=allocatedFromLot(r.id),saldo=Math.max(0,qc-allocated),kg=Number(r.pesoKg||r.peso||0),pc=Number(r.precoCompra||0);return '<tr><td>'+fmtDate(r.data)+'</td><td><b>'+esc(r.loteCodigo||'—')+'</b></td><td>'+esc(r.vendedor||'')+'</td><td>'+esc(r.era||'—')+'</td><td class="num">'+num.format(qc)+'</td><td class="num">'+num.format(allocated)+'</td><td class="num"><b>'+num.format(saldo)+'</b></td><td class="num">'+num.format(daysInStock(r))+'</td><td class="num">'+num.format(kg)+'</td><td class="num">'+money.format(pc)+'</td><td class="num">'+money.format(kg?pc/kg:0)+'</td><td class="num">'+money.format(saldo*pc)+'</td><td><span class="badge warn">'+(saldo===0?'Vendido':'Em estoque')+'</span></td><td>'+esc(r.parceiro||'—')+'</td></tr>';}).join('');}
 function run(){
  if(false)return;
  ...
  if(window.renderStock&&!window.renderStock.__v131Allocated){var rs=window.renderStock;window.renderStock=function(){var out=rs.apply(this,arguments);refreshAllocatedStock();try{if(window.refreshAnimalsV121)window.refreshAnimalsV121()}catch(e){}return out};window.renderStock.__v131Allocated=true}
 ...
 document.addEventListener('submit',function(ev){var f=ev.target,sale=window.__modeV127==='venda'||window.__negListView==='buyer'||(Number(document.getElementById('rqv')?.value||0)>0&&Number(document.getElementById('rqcomp')?.value||0)===0);if(!f||f.id!=='recordForm'||!sale)return;window.__v131SaleEditId=(document.getElementById('rid')||{}).value||'';var total=Number(document.getElementById('rqv')?.value||0),chosen=[];document.querySelectorAll('#loteRows130 .field').forEach(function(row){var l=row.querySelector('.lote130'),q=row.querySelector('.qtdLote130');if(l&&l.value&&Number(q&&q.value||0)>0)chosen.push({lotId:l.value,quantity:Number(q.value)});});if(!chosen.length&&window.__v131PendingLots)chosen=window.__v131PendingLots.map(function(x){return {lotId:x.lotId||x.id,quantity:Number(x.quantity||x.quantidade||0)}});window.__v131SubmitLots=chosen;var sum=chosen.reduce(function(a,x){return a+Number(x.quantity||0)},0);if(total!==sum){ev.preventDefault();ev.stopImmediatePropagation();alert('A quantidade da venda deve ser igual à soma dos animais dos lotes. Venda: '+total+' • Lotes: '+sum);}},true);
  document.addEventListener('submit',function(ev){var sale=window.__modeV127==='venda'||window.__negListView==='buyer'||(Number(document.getElementById('rqv')?.value||0)>0&&Number(document.getElementById('rqcomp')?.value||0)===0);if(ev.target&&ev.target.id==='recordForm'&&sale){function syncSaleLots(){var chosen=(window.__v131SubmitLots||window.__v131PendingLots||[]).map(function(x){return {lotId:x.lotId||x.id,quantity:Number(x.quantity||x.quantidade||0)}}),id=window.__v131SaleEditId||(document.getElementById('rid')||{}).value,list=(typeof records!=='undefined'&&records||window.records||[]),r=list.find(function(x){return x.id===id})||list[list.length-1];if(r&&chosen.length){var desired=Number(r.quantVenda||0),sum=chosen.reduce(function(a,x){return a+x.quantity},0);if(desired>=0&&sum!==desired){var left=desired;chosen=chosen.map(function(x){var q=Math.min(x.quantity,left);left-=q;return {lotId:x.lotId,quantity:q}}).filter(function(x){return x.quantity>0});}r.animalLotsSold=chosen;r.sourceLots=chosen;var chosenTotal=chosen.reduce(function(a,x){return a+Number(x.quantity||0)},0);if(!Array.isArray(r.animalNumbersSold)||!r.animalNumbersSold.length)r.animalNumbersSold=numberedForSale(r,list);else if(r.animalNumbersSold.length>chosenTotal)r.animalNumbersSold=r.animalNumbersSold.slice(0,chosenTotal);r.updatedAt=new Date().toISOString();try{persist()}catch(e){}}try{if(typeof renderAll==='function')renderAll();else if(typeof renderStock==='function')renderStock();if(typeof renderTable==='function')renderTable();}catch(e){}}[150,600,1200,2200,4000].forEach(function(ms){setTimeout(syncSaleLots,ms);});}},false);
...
(function(){
  ...
  function render(){
    ...
    function allocated(id){return records.reduce(function(total,x){var lots=x.animalLotsSold||x.sourceLots||[],used=Array.isArray(lots)?lots.reduce(function(n,z){return n+((z.lotId||z.id)===id?Number(z.quantity||z.quantidade||0):0)},0):0,saleTotal=Number(x.quantVenda||0);return total+(saleTotal>0?Math.min(used,saleTotal):used)},0)}
    body.innerHTML=records.filter(function(r){return Number(r.quantCompra||0)-allocated(r.id)>0}).map(function(r){
      var nums=Array.isArray(a[r.id])?a[r.id]:[], available=nums.filter(function(n){return !sold[String(n)]}), total=Math.max(0,Number(r.quantCompra||0)), soldCount=allocated(r.id), remaining=Math.max(0,total-soldCount);
...
(function(){
 ...
 function allocated(id,excludeId){try{return (typeof records!=='undefined'?records:[]).reduce(function(total,s){if(excludeId&&s.id===excludeId)return total;var lots=saleLots(s),used=lots.reduce(function(a,x){return a+((x.lotId||x.id)===id?Number(x.quantity||x.quantidade||0):0)},0),saleTotal=Number(s.quantVenda||0);return total+(saleTotal>0?Math.min(used,saleTotal):used)},0)}catch(e){return 0}}
...
(function(){
  ...
  window.markPdfDeleteV106=function(key){var c=cfg[key];if(!c)return;var inp=document.getElementById(c.input),st=document.getElementById(c.status);if(!inp||!st)return;inp.dataset.deletePdf='1';inp.value='';try{var id=(document.getElementById('rid')||{}).value||'';var deleted=JSON.parse((window.userGet?userGet('gado_pdf_deletions_v20'):localStorage.getItem('gado_pdf_deletions_v20'))||'{}');deleted[id+'|'+(key==='pay'?'payment':key)] = true;if(window.userSet)userSet('gado_pdf_deletions_v20',JSON.stringify(deleted));else localStorage.setItem('gado_pdf_deletions_v20',JSON.stringify(deleted));var r=currentRecord();if(r){r[c.field]=null;r.updatedAt=new Date().toISOString();if(typeof persist==='function')persist();if(window.scheduleCloudSave)window.scheduleCloudSave();}}catch(e){}st.innerHTML='<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-top:6px"><span class="hint" style="color:#b42318;font-weight:700">PDF será excluído ao salvar</span><button type="button" class="mini" onclick="cancelPdfDeleteV106(\''+key+'\')">Cancelar exclusão</button></div>';};
  window.cancelPdfDeleteV106=function(key){try{var id=(document.getElementById('rid')||{}).value||'';var deleted=JSON.parse((window.userGet?userGet('gado_pdf_deletions_v20'):localStorage.getItem('gado_pdf_deletions_v20'))||'{}');delete deleted[id+'|'+(key==='pay'?'payment':key)];if(window.userSet)userSet('gado_pdf_deletions_v20',JSON.stringify(deleted));else localStorage.setItem('gado_pdf_deletions_v20',JSON.stringify(deleted));}catch(e){}var r=currentRecord();if(r)renderOne(key,r[cfg[key].field]);};
